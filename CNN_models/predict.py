...
MODEL_PATH = "best_wbc_model.pth"
TEST_METADATA = "test_metadata.csv"
TEST_IMG_DIR = "./data_cropped/test"
IMG_SIZE = 256 # 保持与训练一致
...
def predict():
    ...
    with torch.no_grad():
        for _, row in tqdm(test_df.iterrows(), total=len(test_df)):
            ...
            try:
                ...
                # --- TTA 过程 ---
                # 制作图像的 4 个版本：原始, 水平翻转, 垂直翻转, 水平+垂直翻转
                aug_images = [
                    image,
                    F.hflip(image),
                    F.vflip(image),
                    F.hflip(F.vflip(image))
                ]
                
                # 转换为张量 batch
                batch_tensors = torch.stack([test_transform(img) for img in aug_images]).to(DEVICE)
                
                # 推理得到 logits
                outputs = model(batch_tensors) # 形状: (4, 13)
                
                # 计算平均概率 (Softmax 后平均) 或直接平均 Logits (通常 Logits 平均更稳健)
                # 我们采用先 Softmax 再求平均值的策略
    ...
    # 保存
    test_df['label'] = predictions
    test_df[['ID', 'label']].to_csv("submission_resnet50_tta.csv", index=False)

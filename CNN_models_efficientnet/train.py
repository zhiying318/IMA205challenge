...
def train():
    ...
    train_loader, val_loader, class_names = get_loaders(
        ...
        batch_size=64,      # 如果显存够大可以设 64
        img_size=300        # 必须和你 dataset.py 里的 Resize 一致 efficientnet 版本用 300x300 的输入，所以 Resize 也要改成 300x300，resnet 版本则是 256x256
    )
    ...
    for epoch in range(60):
        ...
        plt.savefig(f'./CNN_models_efficientnet/confusion_matrix/confusion_matrix_epoch_{epoch}.png')
        plt.close()
        ...
        if macro_f1 > best_f1:
            best_f1 = macro_f1
            torch.save(model.state_dict(), "best_wbc_model_efficientnet_b3.pth")
